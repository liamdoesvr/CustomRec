    ...
    internal class APIServer
    {
        ...
        private void StartListen()
        {
            try
            {
                ...
                this.listener.Prefixes.Add("http://localhost:" + Menu.Menu.version + "/");
                if (Menu.Menu.version == "2016")
                {
                    for (; ; )
                    {
                        this.listener.Start();
                ...
                if (Menu.Menu.version == "2017")
                {
                    for (; ; )
                    {
                        ...
                        if (Url == "platformlogin/v1/profiles")
                        {
                            ...
                            if (new WebClient().DownloadString("https://raw.githubusercontent.com/liamdoesvr/CustomRec/main/Config/banned.txt").Contains(File.ReadAllText("SaveData\\Profile\\userid.txt")))
                            {
                                Console.WriteLine("You are banned from playing CustomRec. Make a ticket in the Discord to appeal.");
                        ...
                        {
                            s = CustomRec.GameSessions.GameSessions.JoinRandom(text);
                        }
                        if (Url == "gamesessions/v2/create")
                        {
                            s = CustomRec.GameSessions.GameSessions.Create(text);
                        }
                ...
                else if (Menu.Menu.version == "2018")
                {
                    for (; ; )
                    {
                        this.listener.Start();
                        ...
                        if (Url == "gamesessions/v2/joinrandom")
                        {
                            s = GameSessions.GameSessions.JoinRandom(text);
                        }
                        if (Url == "gamesessions/v2/create")
                        {
                            s = GameSessions.GameSessions.Create(text);
                        }
                        ...
                        if (Url == "presence/v3/heartbeat")
                        {
                            if (new WebClient().DownloadString("https://raw.githubusercontent.com/liamdoesvr/CustomRec/main/Config/banned.txt").Contains(File.ReadAllText("SaveData\\Profile\\userid.txt")))
                            {
                                ...
                                Menu.Menu.isBanned = true;
                                WebSocket.Late2018WebSock.instance.Broadcast(API.Notification.Reponse.createBannedResponse());
                            }
                            s = JsonConvert.SerializeObject(Notification2018.Reponse.createResponse(4, c000020.m000027()));
                        }
                        if (Url == "rooms/v1/featuredRoomGroup")
                        {
                            s = new WebClient().DownloadString("https://raw.githubusercontent.com/liamdoesvr/CustomRec/main/Config/dormroomslideshow.txt");
                        }
                        if (Url.StartsWith("rooms/v1/hot"))
                        {
                            s = new WebClient().DownloadString("https://raw.githubusercontent.com/liamdoesvr/CustomRec/main/Config/hotrooms.txt");
                        }
                        ...
                        {
                            s = new WebClient().DownloadString("https://raw.githubusercontent.com/liamdoesvr/CustomRec/main/Config/rcsslideshow.txt");
                        }
                        Console.WriteLine("API Response: " + s);
                        bytes = Encoding.UTF8.GetBytes(s);
                        response.ContentLength64 = (long)bytes.Length;
        ...
        public static string ChallengesV1GetCurrent = "{\"Success\":true,\"Message\":\"CustomRec\"}";
        ...
        public static string Banned = "{\"ReportCategory\":1,\"Duration\":10000000000000000,\"GameSessionId\":100,\"Message\":\"You are banned from playing CustomRec. Make a ticket in the Discord to appeal.\"}";
