			...
			if (dictionary.ContainsKey("api"))
			{
				...
				if (text2 != null)
				{
					if (text2 == "playerSubscriptions/v1/update")
					{
						...
						return JsonConvert.SerializeObject(Notification.Reponse.createResponse(12, GameSessions.GameSessions.StatusSessionInstance()));
					}
					if (text2 == "heartbeat2")
					{
						Console.WriteLine("[WSS] Heartbeat 2 sent by game client.");
						return JsonConvert.SerializeObject(Notification.Reponse.createResponse(4, GameSessions.GameSessions.StatusSessionInstance()));
					}
				}
		...
		public class Reponse
		{
			...
			public static Notification.Reponse createBannedResponse()
			{
				return new Notification.Reponse
				{
					...
					Msg = new CustomRec.API.ModerationBlockDetails()
                    {
						ReportCategory = 1,
